...
#define SLED1734X_PWM_REGISTER_COUNT 64
...
// This is the bit pattern in the LED control registers and PWM Register
// (for matrix type 3, using split frames)
// Offset is 0x020 This is specific for K66V1
//  CHA - control  -      |  b15  b14  b13  b12  b11  b10  b09  b08  b07  b06  b05  b04  b03  b02  b01  b00
//  CHA -          -  PWM |   A    B    C    D    E    F    G    H    I    J    K    L    M    N    O    P
//  CHA -          -  PWM | 0x00 0x01 0x02 0x03 0x04 0x05 0x06 0x07 0x08 0x09 0x0A 0x0B 0x0C 0x0D 0x0E 0x0F
// FRAME 1 --------------------------------------------------------------------------------------------
//  CA1 - 0x00     - 0x20 | R13, R12, R11, R10, R09, R08, R07, R06, R05, R04, R03, R02, R01, R00,  - ,  -
//  CA2 - 0x02     - 0x30 | G13, G12, G11, G10, G09, G08, G07, G06, G05, G04, G03, G02, G01, G00,  - ,  -
//  CA3 - 0x04     - 0x40 | B13, B12, B11, B10, B09, B08, B07, B06, B05, B04, B03, B02, B01, B00,  - ,  -
//  CA4 - 0x06     - 0x50 | R27, R26, R25, R24, R23, R22, R21, R20, R19, R18, R17,  - ,  - , R16, R15, R14
//  CA5 - 0x08     - 0x60 | G27, G26, G25, G24, G23, G22, G21, G20, G19, G18, G17,  - ,  - , G16, G15, G14
//  CA6 - 0x0A     - 0x70 | B27, B26, B25, B24, B23, B22, B21, B20, B19, B18, B17,  - ,  - , B16, B15, B14
//  CA7 - 0x0C     - 0x80 | R41, R40, R39, R38, R37, R36, R35, R34,  - ,  - , R33, R32, R31, R30, R29, R28
//  CA8 - 0x0E     - 0x90 | G41, G40, G39, G38, G37, G36, G35, G34,  - ,  - , G33, G32, G31, G30, G29, G28
// FRAME 2--------------------------------------------------------------------------------------
//  CA9 - 0x00     - 0x20 | B41, B40, B39, B38, B37, B36, B35, B34,  - ,  - , B33, B32, B31, B30, B29, B28
//  CB1 - 0x02     - 0x30 | R55, R54, R53, R52, R51,  - ,  - , R50, R49, R48, R47, R46, R45, R44, R43, R42
//  CB2 - 0x04     - 0x40 | G55, G54, G53, G52, G51,  - ,  - , G50, G49, G48, G47, G46, G45, G44, G43, G42
//  CB3 - 0x06     - 0x50 | B55, B54, B53, B52, B51,  - ,  - , B50, B49, B48, B47, B46, B45, B44, B43, B42
//  CB4 - 0x08     - 0x60 | R69, R68,  - ,  - , R67, R66, R65, R64, R64, R62, R61, R60, R59, R58, R57, R56
//  CB5 - 0x0A     - 0x70 | G69, G68,  - ,  - , G67, G66, G65, G64, G64, G62, G61, G60, G59, G58, G57, G56
//  CB6 - 0x0C     - 0x80 | B69, B68,  - ,  - , B67, B66, B65, B64, B64, B62, B61, B60, B59, B58, B57, B56
//  CB7 - 0x0E     - 0x90 |  - ,  - ,  - ,  - ,  - ,  - ,  - ,  - ,  - ,  - ,  - ,  - ,  - ,  - ,  - ,  -
// --------------------------------------------------------------------------------------
...
void sled1734x_init_drivers(void) {
    ...
    for (uint8_t i = 0; i < SLED1734X_DRIVER_COUNT; i++) {
        sled1734x_init(i);
        sled1734x_direct_write_led_control_registers(i);
    }
...
void sled1734x_direct_write_led_control_registers(uint8_t index) {
    // select the first frame
    sled1734x_select_page(index, SLED1734X_COMMAND_FRAME_1);
    for (int i = 0; i < 16; i++) {
        sled1734x_write_register(index, i, sled1734x_state_frame1[i]);
    }
    // select the second frame
    sled1734x_select_page(index, SLED1734X_COMMAND_FRAME_2);
    for (int i = 0; i < 8; i++) {
        sled1734x_write_register(index, i, sled1734x_state_frame2[i]);
    }
}
...
void sled1734x_set_color(int index, uint8_t red, uint8_t green, uint8_t blue) {
    ...
    if (index >= 0 && index < SLED1734X_LED_COUNT) {
        ...
        driver_buffers[led.driver].pwm_buffer_dirty  = true;
            ...
            location = 0x2C = 44
            */
            uint8_t base      = driver_buffers[led.driver].pwm_buffer[led.r] / 0x10; // 9
            uint8_t remainder = driver_buffers[led.driver].pwm_buffer[led.r] % 0x10; // D
            uint8_t location  = ((base * 0x10) / 0x03) + remainder;                  // 0x3D

            if (driver_buffers[led.driver].pwm_buffer[location] == ((red * 7 / 255) << 5) + ((green * 7 / 255) << 2) + (blue * 3 / 255)) {
                return;
            }
            driver_buffers[led.driver].pwm_buffer[location] = ((red * 7 / 255) << 5) + ((green * 7 / 255) << 2) + (blue * 3 / 255);
            driver_buffers[led.driver].pwm_buffer_dirty     = true;
#endif // SLED1734X_RGB_MATRIX_COLOR_DEPTH
    }
}
