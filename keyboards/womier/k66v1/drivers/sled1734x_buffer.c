...
#   define SLED1734X_PWM_REGISTER_COUNT 256/3
...
//  CHA - reg  -  A    B    C    D    E    F    G    H    I    J    K    L    M    N    O    P
//  CHA - reg  - 0x00 0x01 0x02 0x03 0x04 0x05 0x06 0x07 0x08 0x09 0x0A 0x0B 0x0C 0x0D 0x0E 0x0F
// --------------------------------------------------------------------------------------------
...
#if (SLED1734X_RGB_MATRIX_COLOR_DEPTH == 24)
        memcpy(buf, (driver_buffers[index].pwm_buffer + i), 16);

#elif (SLED1734X_RGB_MATRIX_COLOR_DEPTH == 8)
        uint8_t base   = (i / 0x30)*10;
        uint8_t remain = i % 0x30;
        for (int j = 0; j < 16; j++) {
            switch (remain)
            {
            case 0x00: /* RED */
                buf[j] = ((driver_buffers[index].pwm_buffer[base+j] & 0b11100000) >> 5)*32;
                break;
            case 0x10: /* GREEN */
                buf[j] = ((driver_buffers[index].pwm_buffer[base+j] & 0b00011100) >> 2)*32;
                break;            
            case 0x20: /* BLUE */
                buf[j] = ((driver_buffers[index].pwm_buffer[base+j] & 0b00000011) >> 0)*64;
                break;  
            }
...
#endif //SLED1734X_I2C_PERSISTENCE
    }
...
void sled1734x_set_color(int index, uint8_t red, uint8_t green, uint8_t blue) {
    ...
    if (index >= 0 && index < SLED1734X_LED_COUNT) {
...
#if (SLED1734X_RGB_MATRIX_COLOR_DEPTH == 24)
        if ( driver_buffers[led.driver].pwm_buffer[led.r] == red && 
        ...
        if ( ((driver_buffers[led.driver].pwm_buffer[location] & 0b11100000) >> 5)*32 == red   && 
             ((driver_buffers[led.driver].pwm_buffer[location] & 0b00011100) >> 2)*32 == green && 
             ((driver_buffers[led.driver].pwm_buffer[location] & 0b00000011) >> 0)*64 == blue ) {
            return;
        }
        driver_buffers[led.driver].pwm_buffer[location] = (driver_buffers[led.driver].pwm_buffer[location] & ~0b11100000) | (((red   / 32) << 5) & 0b11100000);
        driver_buffers[led.driver].pwm_buffer[location] = (driver_buffers[led.driver].pwm_buffer[location] & ~0b00011100) | (((green / 32) << 5) & 0b00011100);
        driver_buffers[led.driver].pwm_buffer[location] = (driver_buffers[led.driver].pwm_buffer[location] & ~0b00000011) | (((blue  / 32) << 5) & 0b00000011);
        driver_buffers[led.driver].pwm_buffer_dirty  = true;
#endif //SLED1734X_RGB_MATRIX_COLOR_DEPTH
    }
}
